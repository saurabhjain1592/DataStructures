...
public class Graph {
	...
	private void tropologicalSort(int t[], MutableInteger num, int i, boolean b[]) {
    	b[i] = true;
	...
	public int[] shortestPath(int i) throws Exception {
    	if (i < 0 || i >= this.v) {
        	...
        	MinHeap<VertexDistancePair> minHeap = new MinHeap<VertexDistancePair>(vdp);
            	...
            	if (temp.getD() == Integer.MAX_VALUE) {
                	...
                	for (int q : graph.get(temp.getV())) {
                    	if(!hs.contains(q)){
                        	...
                        	a[q] = minInt(a[q], a[temp.getV()] + w[temp.getV()][q]);
	...
	private static class MinHeap<T extends VertexDistancePair> {
    	...
    	public MinHeap(T a[]) {
        	this.a = (T[]) Array.newInstance(a.getClass().getComponentType(), a.length + 1);
