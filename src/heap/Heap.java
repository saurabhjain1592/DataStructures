...
public class Heap<T extends Comparable<T>> {
	...
	public Heap(int s, Class<? extends T[]> typeParameterClass) {
		...
		a = (T[]) Array.newInstance(typeParameterClass.getComponentType(),
				s + 1);
		last = 0;
	}
	...
	public Heap(T a[]) {
		...
		this.a = (T[]) Array.newInstance(typeParameterClass.getComponentType(),
				a.length + 1);
	...
	private void heapifyDown(int i) {
		while (i < last) {
			if (2 * i <= last) {
				if (2 * i + 1 <= last) {
					T min = a[2 * i + 1].compareTo(a[2 * i]) < 0 ? a[2 * i + 1]
							: a[2 * i];
					if (min.compareTo(a[i]) < 0) {
						int tI = (min.equals(a[2 * i + 1])) ? (2 * i + 1)
								: (2 * i);
						T temp = a[tI];
						a[tI] = a[i];
						a[i] = temp;
